...
void write_data_to_bank(Chip* bank, char* data, char address_bus[12]);
...
void write_data_to_bank(Chip* chip, char* data, char address_bus[12])
{
	char bank_addr[3] = { address_bus[9],address_bus[10],address_bus[11] };
	int bank_index = decode_ras_pins(bank_addr);
